...
class PostingListResource(Resource) :
    ...
    def post(self) :
        ...
        # rekogintion 서비스를 이용해서 object detection 하여 태그 이름을 가져온다.
        tag_list = self.detect_labels(new_file_name, Config.S3_BUCKET)

        print(tag_list)

        # DB의 posting 테이블에 데이터를 넣어야 하고 tag_name 테이블과 tag 테이블에도 데이터를 넣어줘야 한다.
        try :
            connection = get_connection()

            # 1. posting 테이블에 쿼리를 넣어준다.
            ...
            #    리코그니션을 이용해서 받아온 label이 tag_name 테이블에 이미 존재하면 그 아이디만 가져오고,
            #    그렇지 않으면 테이블에 인서트한 후에 그 아이디를 가져온다.
            for tag in tag_list :
                tag = tag.lower()
                query = '''select *
                        from tag_name
            ...
            # 트랜잭션 처리를 위해서 커밋은 테이블 처리를 다 하고나서 마지막에 한 번 해준다.
            # 이렇게 해주면 중간에 다른 테이블에서 문제가 발생하면
            # 모든 테이블이 원상복구(롤백)된다. 이 기능을 '트랜잭션'이라고 한다.
            connection.commit()
    ...
    def get(self) :
        ...
        try :
            connection = get_connection()
            query = '''select p.id postId, p.imgUrl, p.content, u.id userId, u.email, p.createdAt,
...
class PostingResource(Resource) :
    ...
    def get(self, posting_id) :
        ...
        try :
            connection = get_connection()
            query = '''select p.id as postId, p.imgUrl, p.content, u.id as userId, u.email, p.createdAt,
                        count(l.id) as likeCnt, if(l2.id is null, 0, 1) as isLike
                        from posting p
                        join user u
                        on p.userId = u.id
